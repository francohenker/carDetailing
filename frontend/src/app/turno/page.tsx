...
function TurnoPageContent() {
    ...
    const handleConfirmBooking = async (): Promise<void> => {
        ...
        try {
            ...
            //creacion del turno
            const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/turno/create`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${localStorage.getItem("jwt")}`,
                                        ...
                                        <div className="relative bg-base-200 rounded-lg p-0 md:pr-48">
                                            ...
                                            <div className="no-scrollbar inset-y-0 right-0 flex max-h-72 w-full scroll-pb-6 flex-col gap-4 overflow-y-auto border-t p-4 md:absolute md:max-h-none md:w-48 md:border-t-0 md:border-l">
                                                ...
                                                <div className="text-xs bg-blue-50 border border-blue-200 rounded p-2 mb-2">
                                                    <div className="font-medium text-blue-800">Duración total:</div>
                                                    <div className="text-blue-600">
                                                        {Math.floor(bookingData.totalDuration / 60)}h {bookingData.totalDuration % 60}min
                                                    </div>
                                                    <div className="text-blue-500 text-xs mt-1">
                                                        Los horarios consideran este tiempo completo
                                                    </div>
                                                </div>
                                            )} */}
                                                <div className="grid gap-2">
                                                    ...
                                                    {bookingData.date && !slotsLoading && availableSlots
                                                        .filter(slot => slot.available) // Filtrar solo los slots disponibles
                                                        .map((slot) => (
                                                            <Button
                                                                key={slot.id}
